...
class S3DISDataset(CustomDataset): # yaml.data.train이 들어옴
    ...
    def get_filenames(self):
        ...
        filenames_all = []
        for p in self.prefix: # self.prefix = ['Area_1','Area_2,'Area_3' ...]
            filenames = glob(osp.join(self.data_root, p + '*' + self.suffix)) # filenames = [~~~/Area_1/~~.pth]
            assert len(filenames) > 0, f'Empty {p}'
            filenames_all.extend(filenames)
        filenames_all = sorted(filenames_all * self.repeat) #하나의 pth파일이 20개가 복사됨.
