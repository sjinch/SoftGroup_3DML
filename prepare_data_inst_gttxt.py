...
def get_parser():
    ...
    parser.add_argument(
        '--data-dir', type=str, default='./preprocess', help='directory save processed data') #gt이기 때문에 preprocess 하위 디렉토리 참조
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    files = sorted(glob.glob(f'{data_dir}/*.pth')) # files = ['~/1.pth', '~/2.pth', '~/3.pth' ...etc]
    ...
    for i, f in enumerate(files):
        ...
        for inst_id in inst_ids:
            ...
            instance_mask = np.where(instance_labels == inst_id)[0] # instance_labels = 각각의 pcd에 해당하는 라벨
            # instance_mask = inst_id에 해당하는 instance_label들의 index를 반환
