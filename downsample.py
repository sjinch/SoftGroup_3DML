...
if __name__ == '__main__':
    ...
    if voxelize_flag: # default는 voxelize_flag = None
        print('processing: voxelize')
        save_dir = f'{data_dir}_voxelize'
    else: # 여기 실행
        print('processing: random sample')
        save_dir = f'{data_dir}_sample' # 즉, process_sample에 있는것이 downsampling이 적용된것.
    ...
    for data_file in glob.glob(osp.join(data_dir, '*.pth')):
        ...
        (coords, colors, semantic_labels, instance_labels, room_label,
         scene) = torch.load(data_file) #pth파일을 하나씩 open
        ...
        torch.save((coords, colors, semantic_labels, instance_labels, room_label, scene), save_path) # 사실상 1/4의 pcd만 남음.
